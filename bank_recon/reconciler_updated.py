...
class BankReconciler:
    ...
    1. Loading and parsing bank statements and internal records using schema mapping
    ...
    def __init__(self, 
                 ...
                 match_criteria=None,
                 config_path=None,  # Deprecated, kept for backward compatibility
                 logger=None):
        """
        ...
        # Setup logger
        self.logger = logger or logging.getLogger('bank_reconciliation')
        ...
        # Initialize schema mapper, parser and matcher
        self.schema_mapper = SchemaMapper(logger=self.logger)
        self.parser = BankStatementParser(schema_mapper=self.schema_mapper, logger=self.logger)
        ...
        if config_path:
            self.logger.warning("The config_path argument is deprecated and will be ignored. "
                                "BankReconciler now uses SchemaMapper for configuration.")
    ...
    def reconcile(self, bank_statement_path, internal_records_path, 
                 bank=None, sheet_name=0, internal_schema_id=None):  # Added internal_schema_id
        """
        ...
        Args:
            ...
            internal_schema_id (str, optional): Schema ID for internal records.
        ...
        # Parse bank statement using the updated parser
        ...
        # Load and transform internal records using SchemaMapper
        self.logger.info(f"Loading internal records from {internal_records_path}")
        internal_df, internal_process_info = self.schema_mapper.process_internal_records(
            internal_records_path, schema_id=internal_schema_id
        )
